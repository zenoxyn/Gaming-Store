<nav class="bg-[#11042A] shadow-md px-4 py-2 text-white">
  ...
  <div class="md:hidden block">
    <div class="flex items-center justify-between py-2">
      <!-- Logo -->
      <a href="{{ route('home') }}" class="flex items-center space-x-2">
        <img src="{{ asset('images/icons/logo.png') }}" alt="Logo" class="h-10 w-auto" />
        <span class="text-lg font-bold bg-linear-to-r from-yellow-500 via-orange-400 to-purple-500 bg-clip-text text-transparent">FRYN.COM</span>
      </a>
      <!-- Hamburger -->
    ...
    <div id="mobile-menu" class="hidden flex flex-col gap-2 bg-[#1a0b2e] rounded-lg shadow-lg p-4 mt-2 z-50">
      @auth
        @if(auth()->user()->role_user === 'seller' && auth()->user()->seller && auth()->user()->seller->verification_status === 'verified')
          <a href="{{ route('seller.products.create') }}" class="bg-purple-500 text-white px-4 py-3 rounded-xl hover:bg-purple-800 text-sm font-semibold block">Sell Item</a>
        @elseif(auth()->user()->seller && auth()->user()->seller->verification_status === 'pending')
          <span class="bg-yellow-600/20 border border-yellow-600 text-yellow-400 px-4 py-3 rounded-xl text-sm font-semibold cursor-not-allowed block">Pending Verification</span>
        @else
          <a href="{{ route('seller.apply') }}" class="bg-purple-500 text-white px-4 py-3 rounded-xl hover:bg-purple-800 text-sm font-semibold block">Sell Item</a>
        @endif
        <a href="{{ route('chat.index') }}" class="block px-4 py-3 hover:bg-purple-600/20 transition"><i class="ri-message-2-line mr-2"></i>Chat</a>
        <a href="{{ route('dashboard') }}" class="block px-4 py-3 hover:bg-purple-600/20 transition"><i class="ri-dashboard-line mr-2"></i>Dashboard</a>
        <a href="{{ route('wallet.index') }}" class="block px-4 py-3 hover:bg-purple-600/20 transition"><i class="ri-wallet-line mr-2"></i>Wallet: Rp {{ number_format(auth()->user()->wallet->balance ?? 0, 0, ',', '.') }}</a>
        <form action="{{ route('logout') }}" method="POST">
          @csrf
          <button type="submit" class="block w-full text-left px-4 py-3 hover:bg-purple-600/20 transition text-red-400"><i class="ri-logout-box-line mr-2"></i>Logout</button>
        </form>
      @else
        <a href="{{ route('login') }}" class="bg-purple-500 text-white px-4 py-3 rounded-xl hover:bg-purple-800 text-sm font-semibold block">Login</a>
        <a href="{{ route('register') }}" class="px-4 py-3 rounded-xl border border-purple-500 hover:bg-purple-600/20 text-sm font-semibold transition block">Register</a>
      @endauth
      <hr class="my-2 border-purple-700/30">
      <a href="{{ route('products.account') }}" class="hover:text-orange-300 border-b-2 border-transparent hover:border-orange-300 hover:bg-purple-800/5 p-3 transition block {{ request()->is('account') ? 'border-orange-300 text-orange-300' : '' }}"><i class="ri-account-box-line text-xl"></i> Account</a>
      <a href="{{ route('products.ingame') }}" class="hover:text-orange-300 border-b-2 border-transparent hover:border-orange-300 hover:bg-purple-800/5 p-3 transition block {{ request()->is('in-game-items') ? 'border-orange-300 text-orange-300' : '' }}"><i class="ri-sword-line text-xl"></i> In-Game Items</a>
      <a href="{{ route('products.topup') }}" class="hover:text-orange-300 border-b-2 border-transparent hover:border-orange-300 hover:bg-purple-800/5 p-3 transition block {{ request()->is('top-up') ? 'border-orange-300 text-orange-300' : '' }}"><i class="ri-wallet-line text-xl"></i> Top-ups</a>
    </div>
    <script>
